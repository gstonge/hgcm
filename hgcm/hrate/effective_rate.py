...
def get_Gyni(rate,rho,ymax,nmax,pyn):
    """get_cyni returns the stationary cyni
...
def get_hyni(rate,ymax,nmax):
    """get_hyni returns the stationary hyni (near critical point)
...
def get_leading_eigenvector(excess_membership, rate, pn, pyn, ymax, nmax, nb_iter=100, verbose=False,
    ...
    for _ in range(nb_iter):
        ...
        if verbose:
            print(f"Eigenvalue estimate: {ev}")
...
def stationary_effective_rate(rate,rho,pyn,ymax,nmax):
    ...
    given rho and joint rate distribution pyn.
...
def qs_vector_field(v, t, state_meta, rate, pyn, ymax, rho0=10**(-3),
    ...
    where we use the quasi-static (or quasi-steady state) approximation.

    ***Important : to be used with the *gcm* module?, be careful at import for
    conflicts***
