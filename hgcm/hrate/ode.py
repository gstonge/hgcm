...
contagion on higher-order networks with heterogeneous infection rates within
groups.
"""
...
@jit(nopython=True)
def vector_field(v, t, state_meta):
    ...
    Gyni_field[:,2:,:nmax] += itens[:,2:,1:]*Gyni[:,2:,1:]
    #contribution from equal
    Gyni_field += (-itens - (ntens - itens)*(ratetens*itens + rho))*Gyni
    #contribution from below
    Gyni_field[:,2:,1:] += ((ntens[:,2:,:nmax] - itens[:,2:,:nmax])
                            *(ratetens[:,2:,:nmax]*itens[:,2:,:nmax]
                              + rho))*Gyni[:,2:,:nmax]
